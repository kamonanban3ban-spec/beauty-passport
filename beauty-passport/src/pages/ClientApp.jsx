    ...
    <>
      ...
      {loggedIn && (
        <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:420, background:'#fff', borderTop:'1px solid #edd8de', display:'flex', zIndex:100, boxShadow:'0 -4px 20px rgba(0,0,0,0.06)' }}>
          <button onClick={()=>setView('feed')} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4, background:'none', border:'none', cursor:'pointer', padding:'10px 0', color: view==='feed' ? '#e8829a' : '#b89ca4', fontFamily:"'DM Sans', sans-serif", fontSize:10, fontWeight: view==='feed' ? 700 : 500 }}>🏠</span>ホーム</button>
          <button onClick={()=>{ if(records.length===0) return; setAddPhoto(records[0]) }} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4, background:'none', border:'none', cursor:'pointer', padding:'10px 0', color:'#b89ca4', fontFamily:"'DM Sans', sans-serif", fontSize:10, fontWeight:500 }}>📷</span>写真を追加</button>
          <button onClick={()=>setShowQr(true)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4, background:'none', border:'none', cursor:'pointer', padding:'10px 0', color:'#b89ca4', fontFamily:"'DM Sans', sans-serif", fontSize:10, fontWeight:500 }}>📲</span>QRコード</button>
        </div>
      )}
      {showQr && client && (
